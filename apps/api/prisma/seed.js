...
function resolveLocalStoragePath(envPath) {
    if (envPath) {
        ...
        if (envPath.startsWith('apps') && (0, fs_1.existsSync)(apiDir) && cwd === path.dirname(apiDir)) {
...
async function main() {
    const users = [
        { email: 'member@league.test', role: client_1.Role.MEMBER },
        { email: 'agent@league.test', role: client_1.Role.AGENT },
        { email: 'admin@league.test', role: client_1.Role.ADMIN }
    ];
    ...
    const s3 = s3Bucket && shouldUpload ? new client_s3_1.S3Client({ region: s3Region }) : null;
    const localStorageFlag = process.env.LOCAL_STORAGE;
    const localStorage = localStorageFlag === 'true' ||
    ...
    for (let index = 0; index < seeds.length; index += 1) {
        ...
        const sha256 = (0, crypto_1.createHash)('sha256').update(seed.buffer).digest('hex');
        if (localStorage) {
        ...
        const [document, version] = await prisma.$transaction(async (tx) => {
            ...
            const createdVersion = await tx.version.create({
                data: {
                    ...
                    objectKey,
                    sha256,
                    status: client_1.VersionStatus.CLEAN
                }
            });
